...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		character2[0] = (HBITMAP)LoadImage(g_hInst, TEXT("images\\d1.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		character2[1] = (HBITMAP)LoadImage(g_hInst, TEXT("images\\d2.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		character2[2] = (HBITMAP)LoadImage(g_hInst, TEXT("images\\d4.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		character2[3] = (HBITMAP)LoadImage(g_hInst, TEXT("images\\d5.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		character2[4] = (HBITMAP)LoadImage(g_hInst, TEXT("images\\d6.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		character2[5] = (HBITMAP)LoadImage(g_hInst, TEXT("images\\d7.bmp"), IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	...
	case WM_TIMER:
		...
		for (int i{}; i < 30; i++) {
			...
			if (rect_flag and c.x + 40 > f_rect.left and c.x + 40 < f_rect.right and c.y + 90 > f_rect.bottom and c.y < f_rect.bottom) {
				c.y = f_rect.bottom - 90;
			}
		}
		...
		case WM_MOUSEMOVE:
		{
			...
			if (RB_Drag) {
				...
				for (int i{}; i < 30; i++) {
					...
					if (rect_flag and c.x + 40 > f_rect.left and c.x + 40 < f_rect.right and c.y > f_rect.bottom - 100 and c.y + 80 <= f_rect.bottom) {
						c.x += dx;
						c.y += dy;
					}
				}
	...
	case WM_PAINT:
		...
		for (int i{}; i < 30; i++) {
			...
			if (rect_flag and c.x + 40 > f_rect.left and c.x + 40 < f_rect.right and c.y + 90 == f_rect.bottom){
				SelectObject(memdc, character2[c_num]);}
			else
				SelectObject(memdc, character[c_num]);
...
>>>>>>> 51b5a240d905af38d441bad2e25c4a800915908c
